...
// Setup SPI
void alpha::setupSPI(int SSpin)
{
    Serial.println("Initializing SPI...");
    
    SPI.begin();                                                            // intialize SPI in Arduino
    pinMode(SSpin, OUTPUT);                                                 // ensure slave select is set at output
    digitalWrite(SSpin, HIGH);                                              // ensure slave select is set HIGH
    Serial.println("\n");
    delay(10000);                                                           // allow SPI to set up
}
...
bool alpha::on(int SSpin)
{
    ...
    bool OPC=0;                                                             // OPC status (0 - off; 1 - on) assumed off
    
    // command OPC on while its off (three attempts)
    while (OPC==0)
    {
        i=i+1;
        if (i==1)
        ...
        else
        {
            ...
            if (i==2)
            {   // reset OPC and repeat command if second attempt was unsucessful
                Serial.println("Resetting OPC, please wait 65s...");
                Serial.println("\n");
                delay(65000);
            }
            if (i==3)
            {   // close SPI bus if third attempt was unsucessful
...
// Read histogram and return particle data
particleData alpha::getData(int convert, int SSpin)
{
    particleData dat;                                                      // particle data structure
    double conv;
    byte vals[62];                                                         // 62 bits total
    byte command[] = {0x30, 0x30};                                         // command byte to request histogram
    
    // SPI transaction
    Serial.println("Requesting data...");
    SPI.beginTransaction(SPISettings(300000, MSBFIRST, SPI_MODE1));
    digitalWrite(SSpin, LOW);                                              // write slave pin low
    SPI.transfer(command[0]);                                              // send command byte
    delay(100);
